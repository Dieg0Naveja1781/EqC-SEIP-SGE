  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (foldersResponse.data?.success && foldersResponse.data?.carpetas) {
          const sortedFolders = [...foldersResponse.data.carpetas]
            .sort((a, b) => {
        ...
        if (docsResponse.data?.success && docsResponse.data?.documentos) {
          const sortedDocs = [...docsResponse.data.documentos]
            .sort((a, b) => {
              const dateA = new Date(a.fecha_creacion).getTime();
              const dateB = new Date(b.fecha_creacion).getTime();
              return dateB - dateA; // Descendente: más recientes primero
